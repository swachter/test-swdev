...
import eu.swdev.xml.token.Lexer
...
class LexerTest extends FunSuite {
  ...
  val parsers = new Lexer()

  import parsers._
  import XmlPushParsers._

  test ("S") {
    assert(drive(_S, " ") === Success(Nil, Nil))
    assert(drive(_S, "  ") === Success(Nil, Nil))
    assert(drive(_S, "  x") === Success(Nil, 'x' :: Nil))
    assert(drive(_S, "x") === Failure(Nil, Seq('x')))
  }
